...
df_sdg_1718 %>% 
  ...
  # mutate(`2023` = ifelse(iso == "GRD", NA, `2023`)) %>%
  mutate(`2021` = ifelse(iso == "CIV", 1, `2021`)) %>%
  mutate(`2022` = ifelse(iso == "CIV", 1, `2022`)) %>%
  mutate(`2021` = ifelse(iso == "MDG", 1, `2021`)) %>%
  mutate(`2022` = ifelse(iso == "MDG", 1, `2022`)) %>%
  mutate(`2021` = ifelse(iso == "MYS", 1, `2021`)) %>%
  mutate(`2021` = ifelse(iso == "SLE", 0, `2021`))# %>% 
  # mutate(`2021` = ifelse(iso %in% c("ALB", "BIH", "BLR", "SUR"), 
                         # NA, `2021`))
  # mutate(`2022` = `2021`) 
...
df_sdg_1718 <- df_sdg_1718 %>% 
  ...
  # mutate(iso = ifelse(is.na(iso), "NULL",iso)) %>% 
  # the south korea case
  mutate(total  = ifelse(iso == "GRD" & indicator =="statlaw" & year != 2022,NA, total )) 
...
df_sdg_1718 <- df_sdg_1718 %>% 
  mutate(total  = ifelse(iso == "ALB" & indicator =="statlaw" & year == 2021,NA, total )) %>% 
  mutate(total  = ifelse(iso == "BIH" & indicator =="statlaw" & year == 2021,NA, total )) %>% 
  mutate(total  = ifelse(iso == "BLR" & indicator =="statlaw" & year == 2021,NA, total )) %>% 
  mutate(total  = ifelse(iso == "SUR" & indicator =="statlaw" & year == 2021,NA, total )) 
...
saveRDS(df_sdg_1718, file = paste0("Data/",
                                   year(Sys.Date()),
