...
df_press <- readRDS("~/dropbox/PARIS21/R/PRESS/Output/CH/2024-02-01 PRESS 2023 Data for CH.RDS")

## select necessary columns
df_sdg <- df_press %>% 
  select(recipientname, year, usd_disbursement_defl, isocode) 
rm(df_press)
...
df_recipients <- df_recipients %>% 
  rename(rec = recipientname, iso = isocode) 
...
df_recipients_regional %>% 
  select(rec) %>% 
  inner_join(df_recipients_regional_fixed)
...
df_rec_no_iso_all_fixed <- df_recipients_regional_fixed_long
...
save(df_recipients, 
     ...
     file = paste0("data/",
                   format(Sys.Date(), "%Y"), 
                   "/06.1 intermediate fixing multi recipient issues.Rdata")
)
...
if(nrow(df_recipients_without_iso_non_regional) > 0) {
  df_recipients_without_iso_non_regional <-df_recipients_without_iso_non_regional %>%
    mutate(num = c(1:n()))
