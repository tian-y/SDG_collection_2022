...
df_recipients_without_iso_fixed <- df_recipients_without_iso_fixed %>% 
  mutate(iso = NA, 
         m49 = NA)
...
saveRDS(df_recipients_regions ,paste0( "data/",
                                       format(Sys.Date(),"%Y"),
                                       "/06.2 Recipients and region codes.RDS"))

df_sdg <- df_sdg %>% 
  ungroup %>% 
  mutate(id = 1:n()) %>% 
  rename(rec = recipientname)
